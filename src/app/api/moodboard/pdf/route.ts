...
function safeFilename(input: string) {
  const cleaned = (input || "")
    ...
    .replace(/\s+/g, " ")
    .slice(0, 80);
  return cleaned.length ? cleaned : "Moodboard";
}

function isAllowedSupabasePublicMoodboardsUrl(raw: string) {
  try {
    const u = new URL(raw);
    ...
    // Allow ONLY the public moodboards bucket
    if (!u.pathname.includes("/storage/v1/object/public/moodboards/")) return false;
...
export async function GET(req: Request) {
  try {
    ...
    const title = searchParams.get("title") || "Moodboard";

    if (!img1) {
      return NextResponse.json({ error: "Missing img1" }, { status: 400 });
    }

    if (!isAllowedSupabasePublicMoodboardsUrl(img1)) {
